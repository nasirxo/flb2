...
if __name__ == "__main__":
  if "flb2" in os.getcwd(): os.system("git pull") 
  os.system("clear")
    

if __name__ != "__main__":
  for PY_CACHE in glob.glob("*.pyc"): 
     try:os.remove(PY_CACHE)
     except: pass
  if "__pycache__" in os.listdir():
     os.system("rm -rf __pycache__")

if ".licenceid" not in os.listdir():
   with open(".licenceid","w") as LW:
      LW.write(str(uuid.uuid1().hex))
   if "flb2" in os.getcwd(): os.system("cp .licenceid $HOME")


def LENCODE(text):
   key_maps = {}
   ALPHA = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
   A_E = [ALPHA[i] for i in range(len(ALPHA)) if i%2==0]
   A_O = [ALPHA[i] for i in range(len(ALPHA)) if i%2!=0]
   for i in range(len(A_E)):
    key_maps[A_E[i]]=A_O[-(1+i)]
    key_maps[A_O[i]]=A_E[-(1+i)]
   return "".join([key_maps[x] for x in list(text)]) 

def LENDECODE(text):
   key_maps = {}
   ALPHA = "fvzMdWcUqn61sKpEaDTO9LlrmPBIebQNkjoZF35A4RtGCYyXHSwuiV27xgh0J8"
   A_E = [ALPHA[i] for i in range(len(ALPHA)) if i%2==0]
   A_O = [ALPHA[i] for i in range(len(ALPHA)) if i%2!=0]
   for i in range(len(A_E)):
    key_maps[A_E[i]]=A_O[-(1+i)]
...
LICENCE_ID = open(".licenceid").read()
LICENCE_KEY = LENCODE(base64.b16encode(f"{os.uname().release}|{os.uname().version}|{os.uname().machine}|{CPU_CORES}|{LICENCE_ID}".encode("utf-8")).decode("utf-8"))
...
def verifylicence(LICENCE=None):
 try:
  if LICENCE == None: 
    LINP_KEY = readlicence()
  else:
    LINP_KEY = LICENCE.replace(" ",'')
  if LINP_KEY == base64.b64encode(LENDECODE(LICENCE_KEY).encode("utf-8")).decode("utf-8"):
     return 1
  else:
     return 0
 except: return 0
...
if ".licence" not in os.listdir():
  print(f" {W}[{Y}!{W}] YOUR LICENCE ID : {Y}{LICENCE_KEY}")
  print(f" {W}[{Y}!{W}] SEND THIS ID TO DEVELOPER TO GIVE YOU LICENCE KEY\n")
  while True:
    LICENCE_INPUT = input(f" {W}[{G}>{W}] LICENCE KEY : {Y}")
    if verifylicence(LICENCE_INPUT) == 1:
       print(f" {W}[{G}!{W}] {G}LICENCE VERIFICATION SUCESSFULL !")
       with open(".licence","w") as LW:
         LW.write(LICENCE_INPUT)
       if "flb2" in os.getcwd(): os.system("cp .licence $HOME")
       break
    else:
       print(f" {W}[{Y}!{W}] {G}INVALID LICENCE KEY!")


if verifylicence() == 0:
   print(f" {W}[{Y}!{W}] {Y}LICENCE INVALID OR EXPIRED !")
   try: 
     os.remove(".licence")
     os.remove("flb2/.licence")
   except: pass
   exit()
...
class FB:
   ...
   def cookiestring(self):
     sco = {}
     parm =  input(f" {G}[+] {W}Cookie :{Y} ")
     for x in parm.replace(" ",'').split(";"):
         CVAR = x.split("=")
         sco[CVAR[0]] = CVAR[1]
     self.s.cookies.update(sco)
     return sco
   ...
   def getfriends(self,ID=None):
    ...
    if ID != None: 
      if str(ID).isdigit():
       link = self.url.format(f"/profile.php?v=friends&id={ID}")
      else:
       link = self.url.format(f"/{ID}/friends") 
...
if __name__ == '__main__':
   ...
   if os.path.exists("cookie") == False: 
     print(f"""
       ...
       {W}=== (LOGIN OPTIONS) ===
        ...
        {G}[2] LOGIN VIA COOKIE (INPUT)
        {G}[3] LOGIN VIA COOKIE (STRING)        
        {G}[4] LOGIN VIA FLB ACESS KEY
       {W}========================
     """)
     while True:
       ...
       if INP in list("1234"):
         ...
         if INP == '3':
           print(f"{G} [!] {Y}INFO : {W} Enter Cookie String From Browser ")
           f.cookiestring()
           f.savecookie("cookie")
           break
   ...
   while MENUE_MODE == 6:
     ...
     C_C.F = 0
     C_C.C = 0
     FRIEND_ID = input(f"{W} [{G}>{W}] FRIEND ({Y}ID{W}/{Y}USERNAME{W}) : {Y}")
